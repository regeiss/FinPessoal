...
final class OnboardingUITests: XCTestCase {
    ...
    func testOnboardingSkipFunctionality() throws {
        ...
        if onboardingTitle.exists {
            ...
            if skipButton.exists {
                ...
                let loginScreen = app.staticTexts.matching(NSPredicate(format: "label CONTAINS[cd] 'Entrar' OR label CONTAINS[cd] 'Login'")).firstMatch
    ...
    func testOnboardingContentIsVisible() throws {
        ...
        if onboardingTitle.exists {
            ...
            XCTAssertTrue(staticTexts.count > 0, "Onboarding should have text content")
            
            // Check that at least one image is visible
            let visibleImages = images.allElementsBoundByIndex.filter { $0.isHittable }
            XCTAssertTrue(visibleImages.count > 0, "At least one image should be visible")
        }
    }
    ...
    func testOnboardingCompletionPersistence() throws {
        ...
        XCTAssertFalse(onboardingTitle.waitForExistence(timeout: 2), "Onboarding should not appear after completion")
        
        // Should go directly to login or main app
        let loginOrDashboard = app.staticTexts.matching(NSPredicate(format: "label CONTAINS[cd] 'Entrar' OR label CONTAINS[cd] 'Login' OR label CONTAINS[cd] 'Dashboard'")).firstMatch
