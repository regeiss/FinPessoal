...
class SubcategoryManagementService: ObservableObject {
  ...
  func addCustomSubcategory(_ name: String, to category: TransactionCategory) {
    ...
    guard
      !existing.contains(where: { $0.lowercased() == trimmedName.lowercased() })
    else {
  ...
  func removeCustomSubcategory(
    _ name: String,
    from category: TransactionCategory
  ) async {
  ...
  func canRemoveSubcategory(_ name: String, from category: TransactionCategory)
    async -> Bool
  {
  ...
  private func isSubcategoryInUse(
    _ subcategoryName: String,
  ...
  func getSubcategoryUsage(for category: TransactionCategory) async -> [String:
    Int]
  {
    do {
      let transactions = try await transactionRepository.getTransactions(
