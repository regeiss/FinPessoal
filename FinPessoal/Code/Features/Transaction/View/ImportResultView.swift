...
struct ImportResultView: View {
  ...
  @ViewBuilder
  private func statCard(title: String, value: String, color: Color)
    -> some View
  {
    VStack(spacing: 4) {
      Text(value)
  ...
  @ViewBuilder
  private func importedTransactionsSection(_ transactions: [Transaction])
    -> some View
  {
    VStack(alignment: .leading, spacing: 12) {
      ...
      LazyVStack(spacing: 8) {
        ...
        if transactions.count > 5 {
          Text(
            String(
              localized: "import.result.and.more",
              defaultValue:
                "and \(transactions.count - 5) more..."
            )
          )
          .font(.caption)
          .foregroundColor(.secondary)
