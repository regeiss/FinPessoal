...
struct FinPessoalTests {
  ...
  func testSampleDataCollections() async throws {
    ...
    #expect(
      accounts.allSatisfy { !$0.name.isEmpty },
      "All accounts should have names"
    )
    #expect(
      transactions.allSatisfy { !$0.description.isEmpty },
  ...
  func testCurrencyFormatting() async throws {
    ...
    #expect(
      formatted?.contains("1.234,56") == true,
      "Should format currency correctly for Brazilian locale"
    )
  }
  ...
  func testEnvironmentDetection() async throws {
    #expect(
      TestConfiguration.isRunningTests == true,
      "Should detect that tests are running"
    )
  }
}
...
final class FinPessoalXCTests: XCTestCase {
  ...
  func testErrorHandling() throws {
    ...
    let validationError = TestConfiguration.TestError.mockValidationError(
      "Invalid input"
    )
    XCTAssertTrue(
      validationError.localizedDescription.contains("Invalid input")
    )
  }
