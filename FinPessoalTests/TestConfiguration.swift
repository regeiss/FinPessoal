...
struct TestConfiguration {
  ...
  static func assertCurrencyFormat(
    _ amount: Double,
    ...
    if formattedAmount != expectedFormat {
      XCTFail(
        "Currency format mismatch. Expected: \(expectedFormat), Got: \(formattedAmount)",
        file: file,
        line: line
      )
    }
  ...
  static func assertDateWithinRange(
    ...
    if timeDifference > tolerance {
      XCTFail(
        "Date outside expected range. Difference: \(timeDifference) seconds",
        file: file,
        line: line
      )
    }
  ...
  func waitForAsync<T>(
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      DispatchQueue.global().asyncAfter(deadline: .now() + timeout) {
        continuation.resume(
          throwing: TestConfiguration.TestError.mockNetworkError
        )
      }
    }
  }
